...
# -*- coding: utf-8 -*-
"""
@author: Karen Wintersperger
"""


def build_QUBO(scenario_no):
    '''
    build QUBO matrix from scenario number

    Input: 
    scenario_no: scenario number (from 1-10)

    Output: H, offset
    H: upper-triangular Hamiltonian matrix, without offset
    offset: constant offset
    '''
    file = open("Data/problem_data.json")
    prob = json.load(file)[f"scenario_{scenario_no}"]
    # penalty factors are twice the maximum weight
    alpha = 2 * max([max(l) for l in prob["article_reward"]])
    beta = alpha

    qubo, offset = multi_ks_qubo(num_items=len(prob["article_weight"])
                                 , C=prob["knapsack_capacity"]
                                 , weights=prob["article_weight"]
                                 , vals=prob["article_reward"]
                                 , alpha=alpha
                                 , beta=beta
                                 , gamma=1)
    return np.array(qubo), offset
