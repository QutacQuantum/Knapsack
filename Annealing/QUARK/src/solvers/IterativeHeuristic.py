...
class IterativeHeuristic(Solver):
    ...
    def get_device(self, device_option: str) -> Union[DWave, SimulatedAnnealingSampler]:
        ...
        elif device_option == "arn:aws:braket:us-west-2::device/qpu/d-wave/Advantage_system6":
    ...
    def run(self, mapped_problem: dict, device_wrapper: any, config: Config, **kwargs: dict) -> (dict, float):
        ...
        while (numIters < config['iterations']):
            ...
            if device_wrapper.device_name != "simulatedannealer":
                ...
                set = device.sample(bqm, num_reads=config['number_of_reads'], answer_mode="histogram")
            else:
                # This is for D-Wave simulated Annealer
                set = device.sample(bqm, num_reads=config['number_of_reads'])
